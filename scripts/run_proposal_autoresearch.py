...
TASK_ROOT = REPO_ROOT / "proposal_autoresearch"
ARTIFACT_ROOT = REPO_ROOT / "artifacts" / "autoresearch-proposal"
CURRENT_DIR = ARTIFACT_ROOT / "current"
RUNS_DIR = ARTIFACT_ROOT / "runs"
SEED_FILES = (
    ".python-version",
    "proposal_autoresearch",
    "inputs",
    "pyproject.toml",
    "uv.lock",
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run an autoresearch-style loop over proposal_autoresearch/plot.py.")
    ...
    parser.add_argument(
        "--workspace-root",
        default="/tmp/autoresearch-proposal",
...
def make_run_id(explicit: str) -> str:
    ...
    return f"{stamp}-proposal-autoresearch"


def refresh_repo_baseline(progress_path: Path) -> None:
    append_progress(
        progress_path,
        "Refreshing proposal baseline with `uv run python proposal_autoresearch/plot.py` and `uv run python proposal_autoresearch/prepare.py`.",
    )
    plot_result = run(["uv", "run", "python", "proposal_autoresearch/plot.py"], cwd=REPO_ROOT)
    if plot_result.returncode != 0:
        raise RuntimeError(
            "Refreshing baseline plot failed.\n"
            f"stdout:\n{plot_result.stdout}\n\nstderr:\n{plot_result.stderr}"
        )
    prepare_result = run(["uv", "run", "python", "proposal_autoresearch/prepare.py"], cwd=REPO_ROOT)
...
def seed_run_baseline(run_root: Path) -> tuple[Path, Path]:
    ...
    copy_file(TASK_ROOT / "plot.py", baseline_root / "plot.py")
    copy_tree(CURRENT_DIR, baseline_root / "current")
    copy_file(TASK_ROOT / "plot.py", best_root / "plot.py")
...
Improve `proposal_autoresearch/plot.py` so the fixed evaluator in `proposal_autoresearch/prepare.py` gives the figure a higher score.

## Rules

- Edit only `proposal_autoresearch/plot.py`.
- Do not edit `proposal_autoresearch/prepare.py`, `proposal_autoresearch/program.md`, or files outside `artifacts/autoresearch-proposal/current/`.
...
def seed_iteration_workspace(workspace: Path, best_root: Path) -> None:
    ...
    copy_file(best_root / "plot.py", workspace / "proposal_autoresearch" / "plot.py")
    copy_tree(best_root / "current", workspace / "artifacts" / "autoresearch-proposal" / "current")
    ...
    subprocess.run(["git", "config", "user.name", "proposal-autoresearch"], cwd=workspace, check=True)
    subprocess.run(["git", "config", "user.email", "proposal-autoresearch@example.invalid"], cwd=workspace, check=True)
...
Read:
- `AGENTS.md`
- `proposal_autoresearch/program.md`
- `artifacts/autoresearch-proposal/current/review.md`
- `artifacts/autoresearch-proposal/current/review.json`
- `proposal_autoresearch/plot.py`
...
Task:
- make one coherent improvement pass by editing only `proposal_autoresearch/plot.py`
...
Do not:
- edit `proposal_autoresearch/prepare.py`
- edit `proposal_autoresearch/program.md`
- invoke your own reviewer or change the scoring rubric

When you stop, leave `proposal_autoresearch/plot.py` in the best state you reached this iteration.
""".strip()
...
def evaluate_candidate(workspace: Path, iteration_root: Path) -> tuple[subprocess.CompletedProcess[str], subprocess.CompletedProcess[str]]:
    plot_result = run(["uv", "run", "python", "proposal_autoresearch/plot.py"], cwd=workspace)
    ...
    prepare_result = run(["uv", "run", "python", "proposal_autoresearch/prepare.py"], cwd=workspace)
...
def archive_iteration_state(workspace: Path, iteration_root: Path) -> None:
    current_source = workspace / "artifacts" / "autoresearch-proposal" / "current"
    current_dest = iteration_root / "current"
    copy_tree(current_source, current_dest)
    copy_file(workspace / "proposal_autoresearch" / "plot.py", iteration_root / "plot.py")
