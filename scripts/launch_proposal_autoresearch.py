...
RUNS_DIR = REPO_ROOT / "artifacts" / "autoresearch-proposal" / "runs"
TMUX_PREFIX = "autoresearch-proposal"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Launch a proposal autoresearch run inside tmux.")
...
def build_wrapper(run_id: str, run_root: Path, workspace_root: str, iterations: int, refresh_baseline: bool) -> Path:
    ...
    command = (
        f"uv run python scripts/run_proposal_autoresearch.py "
...
{{
  printf '[%s] launching proposal autoresearch\\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')"
  ...
  printf '[%s] proposal autoresearch exited with code %s\\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" "$exit_code"
  exit "$exit_code"
