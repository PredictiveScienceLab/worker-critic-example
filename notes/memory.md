...
- The first proposal autoresearch run found a stable local optimum around `8.2 / 10`; the limiting failure mode was not proposal fidelity but figure discipline, especially too many visible words, too much small text, and a center hero that still read as method labels rather than an immediate fused reconstruction.
- To push proposal figures further, the task needs hard design constraints, not just soft review language: explicit body-word budget, minimum non-title font size, limit on long text blocks, aim-strip compression, and a requirement that the center hero visually depict the posterior reconstruction.
- Proposal autoresearch baseline refreshes can hang indefinitely inside `proposal_autoresearch/prepare.py` if the Codex reviewer call stalls; the evaluator should use a hard timeout and bounded retry loop so fresh runs fail fast instead of sitting in `starting` forever.
