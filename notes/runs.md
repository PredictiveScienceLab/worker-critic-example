...
- Started a fresh proposal autoresearch run as `20260408-114614-proposal-autoresearch`, but it stalled in `state: starting` because the baseline refresh was blocked inside `proposal_autoresearch/prepare.py` waiting indefinitely for a Codex review call to return.
- Added a hard timeout and bounded retry loop to `proposal_autoresearch/prepare.py` so future baseline refreshes and iteration evaluations fail fast instead of hanging the entire search before iteration 1.
